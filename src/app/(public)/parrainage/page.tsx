...
import { CopyReferralCode } from "./_components/copy-button";

export default async function ReferralPage() {
  const session = await auth();
  if (!session?.user) redirect("/connexion?callbackUrl=/parrainage");

  const [user, referrals, bonusTxs, signupBonus, paymentBonus] = await Promise.all([
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          <h1 className="font-display text-3xl font-bold">
            <Gift className="mr-2 inline h-7 w-7 text-primary" />
            Parrainage
          </h1>
