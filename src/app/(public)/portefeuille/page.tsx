...
import { DepositDialog, WithdrawDialog } from "./_components/wallet-actions";

export default async function WalletPage() {
  const session = await auth();
  if (!session?.user) redirect("/connexion?callbackUrl=/portefeuille");

  const [user, txs, pendingDeposits, pendingWithdrawals, referralStats] = await Promise.all([
                ...
                {txs.map((tx) => {
                  ...
                  return (
                    <li key={tx.id} className="flex items-center justify-between py-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`rounded-full p-2 ${
                            isCredit ? "bg-emerald-500/20 text-emerald-300" : "bg-destructive/20 text-destructive"
                          }`}
                        >
                      ...
                      <p className={`font-semibold ${isCredit ? "text-emerald-400" : "text-foreground"}`}>
                        {isCredit ? "+" : ""}
                        {formatXAF(tx.amount)}
                      </p>
                    </li>
