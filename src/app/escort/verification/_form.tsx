...
import { initiateVerificationAction } from "@/lib/actions/payments";
          ...
          <KpayPayModal
            trigger={
              <Button variant="outline" size="lg" className="w-full" type="button">
                💳 Payer la vérification ({verificationPrice} FCFA)
              </Button>
            }
            title="Paiement de la vérification d'identité"
            description="Payez d'abord les frais de vérification, puis cliquez sur 'Envoyer pour vérification'."
            amount={verificationPrice}
            defaultPhone={defaultPhone}
            initiate={(phone) => initiateVerificationAction({ phone })}
          />
          <Button onClick={submit} disabled={pending} size="lg" className="w-full">
            {pending && <Loader2 className="h-4 w-4 animate-spin" />}
            Envoyer pour vérification (après paiement)
          </Button>
        </div>
      </CardContent>
    </Card>
